...
const buildCategoryTree = (categories: any[]) => {
  ...
  categories.forEach(category => {
    if (category.parent === false) {
      ...
      categoryMap.get(category.parent).children.push(categoryMap.get(category.nav_id));
    }
  });
    ...
    <ul>
        <li class="navigation-mobile__item navigation-mobile__item--expanded">
            <span class="as-link" @click="toggleSubmenu">
                <span class="navigation-mobile__item__icon navigation-mobile__item__icon--icon-page-1">
            ...
            <ul class="navigation-mobile__nested" :class="{ 'expanded': isExpanded }">
                ...
                <li v-for="category in categoriesFull" :key="category.nav_id">
                    <RouterLink :to="getCategoryPath(category)">
                        {{ category.title }}
                    </RouterLink>
                    <!-- Подкатегории (если есть) -->
                    <ul v-if="category.children && category.children.length > 0">
                        <li v-for="subcategory in category.children" :key="subcategory.nav_id">
                            <RouterLink :to="getCategoryPath(subcategory)" itemprop="url">
                                {{ subcategory.title }}
                            </RouterLink>
                            <!-- Подкатегории третьего уровня (если есть) -->
                            <ul v-if="subcategory.children && subcategory.children.length > 0">
                                <li v-for="subsubcategory in subcategory.children" :key="subsubcategory.nav_id">
                                    <RouterLink :to="getCategoryPath(subsubcategory)" itemprop="url">
        ...
        <li class="navigation-mobile__item">
          <RouterLink to="/Statji/StatjiPage">
                <span class="navigation-mobile__item__icon navigation-mobile__item__icon--icon-page-7"><img src="/image/a16de5.svg" width="28" height="28" alt="Перейти к Статьи" loading="lazy" /></span>
                <span class="navigation-mobile__item__title">Статьи</span>
          </RouterLink>
        </li>

        <li class="navigation-mobile__item">
            <RouterLink to="/Garantiya/GarantiyaPage">
                <span class="navigation-mobile__item__icon navigation-mobile__item__icon--icon-page-4"><img src="/image/4d5a09.svg" width="28" height="28" alt="Перейти к Гарантия" loading="lazy" /></span>
                <span class="navigation-mobile__item__title">Гарантия</span>
            </RouterLink>
        </li>

        <li class="navigation-mobile__item">
            <RouterLink to="/Oplata/OplataPage">
                <span class="navigation-mobile__item__icon navigation-mobile__item__icon--icon-page-3"><img src="/image/d159ba.svg" width="28" height="28" alt="Перейти к Оплата" loading="lazy" /></span>
                <span class="navigation-mobile__item__title">Оплата</span>
            </RouterLink>
        </li>

        <li class="navigation-mobile__item">
            <RouterLink to="/Dostavka/DostavkaPage">
                <span class="navigation-mobile__item__icon navigation-mobile__item__icon--icon-page-2"><img src="/image/3ff0cd.svg" width="28" height="28" alt="Перейти к Доставка" loading="lazy" /></span>
                <span class="navigation-mobile__item__title">Доставка</span>
            </RouterLink>
        </li>

        <li class="navigation-mobile__item">
            <RouterLink to="/Okompanii/OkompaniiPage">
                <span class="navigation-mobile__item__icon navigation-mobile__item__icon--icon-page-5"><img src="/image/8774d1.svg" width="28" height="28" alt="Перейти к О компании" loading="lazy" /></span>
                <span class="navigation-mobile__item__title">О компании</span>
            </RouterLink>
        </li>

        <li class="navigation-mobile__item">
            <RouterLink to="/Kontakty/KontaktyPage">
                <span class="navigation-mobile__item__icon navigation-mobile__item__icon--icon-page-6"><img src="/image/ec4772.svg" width="28" height="28" alt="Перейти к Контакты" loading="lazy" /></span>
