...
// Используем composable для работы с globals
const { loadGlobals } = useGlobalsSSG();

const fetchGlobalData = async () => {
  try {
    const globalsData = await loadGlobals();
    if (globalsData && globalsData.favicon) {
      updateFavicon(globalsData.favicon);
    }
  } catch (error) {
...
<template>
  ...
  <GlobalSeoChecker />
  <SeoManager />
  <!-- Убираем v-app для SSG и используем обычный div -->
...
<style scoped>
/* Базовая структура для замены v-app */
.app-layout {
  min-height: 100vh;
  display: flex;
